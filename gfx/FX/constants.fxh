...
static const float3 DayAmbientMapPosX = float3(0.1, 0.1, 0.1); 
static const float3 DayAmbientMapNegX = float3(0.15, 0.15, 0.15); 
static const float3 DayAmbientMapPosY = float3(0.05, 0.05, 0.05); 
static const float3 DayAmbientMapNegY = float3(0.0, 0.0, 0.0); 
static const float3 DayAmbientMapPosZ = float3(0.1, 0.1, 0.1); 
static const float3 DayAmbientMapNegZ = float3(0.03, 0.03, 0.03);
...
static const float COLORMAP_OVERLAY_STRENGTH     = 0.35f; 
static const float COLORMAP_MUD_OVERLAY_STRENGTH = 0.25f;
...
static const float GB_OPACITY_FAR = 0.98f;  // Transparency when camera is far
...
static const float GB_FIRST_LAYER_PRIORITY = 0.95f; // Priority for first/second layer when both are active at the same pixel
